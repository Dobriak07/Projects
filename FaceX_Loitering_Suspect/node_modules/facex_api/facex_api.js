...
const facexAPI = class {
    ...
    createPerson = async (options = {}) => {
        try {
            ...
            let data = {
                ...
                middle_name: options.middle_name ? `${options.middle_name}` : "",
                last_name: options.last_name ? `${options.last_name}` : "",
                notes: options.notes ? `${options.notes}` : "",
                list_id: options.list_id ? options.list_id : 0
            };
    ...
    movePerson = async (listId, personId) => {
        try {
            const response = await fetch(`http://${this.ip}:${this.port}/v1/spotter/list/${listId}?action=include_person&operator=SampleOp`, {
                method: 'POST',
                body: JSON.stringify({person_id: personId})
    ...
    addAnnotetedPhoto = async (id, object) => {
        try {
            let form = new FormData();
            form.append('data', JSON.stringify({hint_bbox: object.hint_bbox}), {contentType: 'application/json'});
            form.append('image', object.image, {contentType: 'image/jpeg'});
    ...
    deletePersonFromList = async (listId, personId) => {
        try {
            const response = await fetch(`http://${this.ip}:${this.port}/v1/spotter/list/${listId}?action=exclude_person&operator=SampleOp`, {
                method: 'POST',
                body: JSON.stringify({person_id: personId})
            });
            return response;
