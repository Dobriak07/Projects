...
function tabloBuild(str, address) {
    if(address) {
    ...
    for(let item of arr) {
        if(item.length == 2 && Object.keys(tabloEffects).includes(item.toLowerCase())) {
            buff.push(Buffer.from(`ff${tabloEffects[item.toLowerCase()]}`, 'hex'));
        }
        else {
...
module.exports = {tabloBuild, setupClock};